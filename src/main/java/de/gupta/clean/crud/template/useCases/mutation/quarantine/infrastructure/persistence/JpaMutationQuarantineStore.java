...
public class JpaMutationQuarantineStore implements MutationQuarantineRepository
{
	...
	private MutationQuarantinePersistenceModel toPersistenceModel(final MutationQuarantineRecord record)
	{
		...
		persistenceModel.setCorrelationId(record.correlationId().map(MutationCorrelationId::value).orElse(null));
		persistenceModel.setCausationId(record.causationId().map(MutationCausationId::value).orElse(null));
	...
	private MutationQuarantineRecord toDomainModel(final MutationQuarantinePersistenceModel persistenceModel)
	{
		return new MutationQuarantineRecord(
				...
				Optional.ofNullable(persistenceModel.correlationId()).map(MutationCorrelationId::new),
				Optional.ofNullable(persistenceModel.causationId()).map(MutationCausationId::new),
